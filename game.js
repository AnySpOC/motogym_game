...
const blackPlasticMat = new THREE.MeshStandardMaterial({ color: 0x090c0f, roughness: 0.62, metalness: 0.05 });
const glassMat = new THREE.MeshStandardMaterial({ color: 0x89d7ff, roughness: 0.08, metalness: 0.08, transparent: true, opacity: 0.64 });
...
function createCone(x, z, turn = "neutral") {
  ...
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.58, 0.64, 0.12, 32), blackPlasticMat);
  ...
  const lowerBand = new THREE.Mesh(new THREE.CylinderGeometry(0.32, 0.4, 0.07, 28), coneBandMat);
  lowerBand.position.y = 0.51;
  lowerBand.castShadow = true;
  const upperBand = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.25, 0.06, 28), coneBandMat);
  upperBand.position.y = 0.83;
  ...
  const arrowHead = new THREE.Mesh(new THREE.ConeGeometry(0.13, 0.26, 3), arrowMat);
  arrowHead.position.set(turn === "right" ? 0.55 : -0.55, 0.09, -0.28);
  arrowHead.rotation.set(Math.PI / 2, 0, turn === "right" ? -0.55 : 2.6);
  arrowHead.visible = turn === "right" || turn === "left";
  group.add(base, cone, lowerBand, upperBand, turnArrow, arrowHead);
...
function makeBike() {
  ...
  const engineFinGroup = new THREE.Group();
  for (let i = 0; i < 5; i += 1) {
    const fin = new THREE.Mesh(new THREE.BoxGeometry(0.66, 0.018, 0.34), rimMat);
    fin.position.set(0, 0.94 + i * 0.045, -0.12);
    fin.castShadow = true;
    engineFinGroup.add(fin);
  }
  ...
  const subFrameLeft = new THREE.Mesh(new THREE.CylinderGeometry(0.028, 0.028, 1.16, 10), frameMat);
  subFrameLeft.position.set(-0.34, 1.02, 0.56);
  subFrameLeft.rotation.set(-0.55, 0, 0.12);
  subFrameLeft.castShadow = true;
  const subFrameRight = subFrameLeft.clone();
  subFrameRight.position.x = 0.34;
  subFrameRight.rotation.z = -0.12;
  ...
  const rearFender = new THREE.Mesh(new THREE.BoxGeometry(0.64, 0.04, 0.72), blackPlasticMat);
  rearFender.position.set(0, 1.02, 1.38);
  rearFender.rotation.x = -0.24;
  rearFender.castShadow = true;

  const wheelGeo = new THREE.TorusGeometry(0.42, 0.095, 18, 42);
  ...
  const frontCaliper = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.16, 0.06), redMat);
  frontCaliper.position.set(0.04, 0.12, -0.23);
  frontCaliper.castShadow = true;
  const spokeMat = new THREE.MeshStandardMaterial({ color: 0xe0e5ea, roughness: 0.28, metalness: 0.72 });
  for (let i = 0; i < 8; i += 1) {
    const spoke = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.02, 0.46), spokeMat);
    spoke.rotation.y = Math.PI / 2;
    spoke.rotation.z = (Math.PI / 8) * i;
    spoke.castShadow = true;
    frontWheel.add(spoke);
  }
  frontWheel.add(frontTire, frontRim, frontDisc, frontCaliper);
  ...
  const chain = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.045, 1.45), blackPlasticMat);
  chain.position.set(-0.36, 0.62, 0.55);
  chain.rotation.x = 0.06;
  chain.castShadow = true;
  const rearShock = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.84, 14), redMat);
  rearShock.position.set(0.34, 1.02, 0.72);
  rearShock.rotation.set(0.72, 0, -0.18);
  rearShock.castShadow = true;
  const footPegLeft = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.045, 0.08), metalMat);
  footPegLeft.position.set(-0.52, 0.88, 0.08);
  footPegLeft.castShadow = true;
  const footPegRight = footPegLeft.clone();
  footPegRight.position.x = 0.52;
  ...
  const leftBlinker = new THREE.Mesh(new THREE.SphereGeometry(0.06, 12, 8), yellowMat);
  leftBlinker.position.set(-0.52, 1.2, -1.34);
  leftBlinker.castShadow = true;
  const rightBlinker = leftBlinker.clone();
  rightBlinker.position.x = 0.52;
  ...
  const pelvis = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.2, 0.34), riderMat);
  pelvis.position.set(0, 1.18, 0.18);
  pelvis.rotation.x = 0.34;
  pelvis.castShadow = true;
  const rider = new THREE.Mesh(new THREE.CapsuleGeometry(0.24, 0.66, 10, 18), riderMat);
  rider.position.set(0, 1.58, -0.03);
  rider.rotation.x = 0.52;
  rider.castShadow = true;
  const chest = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.18, 0.46), suitAccentMat);
  chest.position.set(0, 1.66, -0.24);
  chest.rotation.x = 0.56;
  chest.castShadow = true;
  const backPad = new THREE.Mesh(new THREE.BoxGeometry(0.44, 0.14, 0.52), suitAccentMat);
  ...
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.1, 0.18, 16), riderMat);
  neck.position.set(0, 1.9, -0.26);
  ...
  helmet.scale.set(0.94, 1.04, 1.08);
  ...
  const visor = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.1, 0.04), glassMat);
  visor.position.set(0, 2.04, -0.64);
  visor.castShadow = true;
  const leftArm = new THREE.Mesh(new THREE.CapsuleGeometry(0.055, 0.5, 8, 12), suitAccentMat);
  leftArm.position.set(-0.34, 1.46, -0.56);
  leftArm.rotation.set(1.3, 0.08, -0.28);
  ...
  const leftGlove = new THREE.Mesh(new THREE.SphereGeometry(0.08, 12, 8), blackPlasticMat);
  leftGlove.position.set(-0.46, 1.3, -0.84);
  leftGlove.scale.set(1.1, 0.75, 0.85);
  leftGlove.castShadow = true;
  const rightGlove = leftGlove.clone();
  rightGlove.position.x = 0.46;
  const leftLeg = new THREE.Mesh(new THREE.CapsuleGeometry(0.08, 0.58, 8, 12), suitAccentMat);
  leftLeg.position.set(-0.32, 1.02, 0.22);
  leftLeg.rotation.set(1.02, -0.06, -0.24);
  ...
  const leftBoot = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.11, 0.36), blackPlasticMat);
  leftBoot.position.set(-0.39, 0.78, -0.32);
  leftBoot.rotation.x = 0.34;
  leftBoot.castShadow = true;
  const rightBoot = leftBoot.clone();
  rightBoot.position.x = 0.38;

  riderRoot.add(pelvis, rider, chest, backPad, spinePad, neck, helmet, helmetStripe, visor, leftArm, rightArm, leftShoulder, rightShoulder, leftElbow, rightElbow, leftGlove, rightGlove, leftLeg, rightLeg, leftKnee, rightKnee, leftBoot, rightBoot);
  bike.add(body, tank, tankStripe, headlight, engine, engineFinGroup, cylinderBlock, radiator, frameLeft, frameRight, subFrameLeft, subFrameRight, seat, tail, tailLamp, plate, rearFender, frontWheel, rearWheel, fork, swingArm, chain, rearShock, footPegLeft, footPegRight, exhaust, bar, leftMirror, rightMirror, leftBlinker, rightBlinker, riderRoot);
